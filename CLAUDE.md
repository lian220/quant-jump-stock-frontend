...
### 점수 시스템 (BETA) — ADR 0006 (0~100 재설계)

**현재 상태** (2026-06-07): 점수 모델 0~100 단일 스케일로 재설계됨.
SSoT = `scoring_spec.yaml` (백엔드). **프론트는 점수/등급을 재계산하지 않고 API 값을 표시만 한다 (ADR 0006 §2.8).**

**점수 계산식** (백엔드 산출, 참고용):
```text
# 각 축 0~1 정규화 후 weight 가중합 → ×100 (composite_max = 100)
composite = (0.5·(tech/3.5) + 0.3·(ai/10) + 0.2·(sentiment/10)) × 100
# 결측 축은 제외 + 남은 weight 재정규화 (0점 벌점 금지)
```

**등급 기준**: 백엔드 `compositeGrade`(S/A/B/C/D) **그대로 사용**. percentile 재보정값(scoring_spec.yaml SSoT):
S≥82 / A≥77 / B≥68 / C≥58 / D<58. **FE에 임계 하드코딩 금지** (구 `TIER_THRESHOLDS`·`COMPOSITE_SCORE_GRADE_THRESHOLDS` 제거됨).

```typescript
// getScoreGrade 는 숫자 점수가 아니라 백엔드 compositeGrade('S'|'A'|'B'|'C'|'D') 를 입력받아
// { grade, label, color, badge:'BETA' } 매핑만 한다. 점수→등급 판정은 백엔드 책임.
```

**XAI (§2.9)**: 응답의 `axisContributions`(축별 0~100 기여)·축 원점수로 "각 신호 강도(0~100)" 막대를 표시
(`components/predictions/AxisContributionBars.tsx`). `scoreCoverage`로 결측 커버리지 표기.

### Admin 관리 계획
등급 임계는 운영 분포로 재보정(현 잠정값, 표본 누적 시 재산출). Backoffice Admin 동적 관리 예정.
...
// 등급 매핑 — 백엔드 compositeGrade(S/A/B/C/D)를 입력 (점수 아님)
const scoreGrade = getScoreGrade(stock.compositeGrade);
// { grade: 'S', label: '매우 강한 매수', color: 'text-emerald-400', badge: 'BETA' }
```
